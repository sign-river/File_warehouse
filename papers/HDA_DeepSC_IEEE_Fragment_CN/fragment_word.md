---
title: "面向不完美CSI与可控语义丢包的混合数字—模拟语义图像传输：鲁棒接收、可靠性融合与交互式实验平台"
author:
  - "作者姓名（待填写）"
date: "中文可编辑论文片段"
lang: zh-CN
...
**关键词：** 语义通信；混合数字—模拟传输；不完美信道状态信息；分组错误率；语义特征擦除；可靠性融合

# 1 引言

语义通信不再仅以逐比特无差错恢复为唯一目标，而是关注接收端对任务相关信息和感知质量的保持。深度联合源信道编码能够在低信噪比和信道失配条件下实现渐进式质量退化，HDA-DeepSC进一步将关键语义基描述映射到数字支路，将难以量化的连续残差映射到模拟支路，从而兼顾标准数字链路的可靠性与模拟传输的柔性退化特征 [@xie2021deepsc; @bourtsoulatze2019deepjscc; @xie2025hda]。
...
1. 构建接收域不确定性感知软解调方法，并解释为何简单替换线性均衡器未必带来译码增益；
2. 建立从BER、PER和FER到语义特征不可用率的分层可靠性评价体系；
3. 区分物理层人工擦除与译码后语义特征包擦除，避免将自然链路错误与人为控制变量混为一谈；
4. 引入数字—模拟相对权重与可选特征恢复策略，并通过交互界面实现参数配置、结果可视化和实验记录。
...
数字—模拟分配模块将其分解为量化数字基描述 $\mathbf{z}_D^q$ 和模拟残差 $\mathbf{z}_A$：
...
![统一实验框架。操作者界面同时控制信道、CSI误差、接收机、人工擦除模式、数字—模拟融合权重及恢复策略，并输出链路层与图像层指标。](figures/framework.png){width=6.8in}
...
\mathcal{M}_C(s) &=-\frac{|y-\hat h s|^2}{2v(s)}-\frac{1}{2}\ln v(s),
\end{aligned}
$$
...
$$
L(b_k)=
\log\sum_{s\in\mathcal{S}_{k,1}}\exp[\mathcal{M}_C(s)]
-
\log\sum_{s\in\mathcal{S}_{k,0}}\exp[\mathcal{M}_C(s)].
$$

对于BPSK和QPSK等恒模星座，候选点能量相同，方差修正主要体现为统一缩放；对于16-QAM等非恒模星座，不同候选点具有不同能量，上述度量能够改变候选概率与软信息置信度。
...
$$
\mathrm{PER}=\frac{1}{N_p}\sum_{p=1}^{N_p}\mathbf{1}(\hat{\mathbf b}_p\neq\mathbf b_p),
$$

$$
\mathrm{FER}=\frac{1}{N_f}\sum_{f=1}^{N_f}\mathbf{1}(\hat{\mathbf b}_f\neq\mathbf b_f).
$$
...
$$
\tilde{\mathbf b}_p=m_p^{\mathrm{phy}}\hat{\mathbf b}_p.
$$
...
语义擦除发生在数字比特恢复为量化特征之后。每32个特征值组成一个语义特征包，第 $g$ 组的擦除掩码为 $m_g^{\mathrm{sem}}$。采用恢复值 $\mathbf r_g$ 时，送入融合模块的数字特征为

$$
\tilde{\mathbf z}_{D,g}=m_g^{\mathrm{sem}}\hat{\mathbf z}_{D,g}
+(1-m_g^{\mathrm{sem}})\mathbf r_g.
$$
...
$$
R_{\mathrm{unavail}}=
\frac{|\mathcal N\cup\mathcal A|}{N_p}
=
...
$$
\hat{\mathbf z}=2\alpha_d\hat{\mathbf z}_{\mathrm{base}}
+2(1-\alpha_d)\hat{\mathbf z}_A.
$$

当 $\alpha_d=0.5$ 时，上式严格退化为原始等权相加；$\alpha_d>0.5$ 强调数字基描述，$\alpha_d<0.5$ 则降低不可靠数字特征对最终潜在表示的污染。该参数本身不是完整的自适应融合算法，但为后续利用平均 $|L|$、校验失败标记、有效SNR和包级可靠性预测权重提供了可控基线。
...
实验组A使用完美CSI和匹配软解调；组B使用不完美CSI但忽略估计不确定性的失配软解调；组C使用相同估计信道和误差方差的不确定性感知软解调。每次前向传播前恢复相同随机状态，保证B、C两组共享输入图像、真实衰落、热噪声和信道估计误差。定义
...
在16-QAM工作区间，已有受控NMSE实验结果如表2和图2所示。

**表2 16-QAM区间的平均受控NMSE结果**
...
![受控信道估计误差下的不确定性感知接收机平均收益。NMSE为1.0时BER改善仍为正，但其向图像质量的转化几乎消失。](figures/nmse_results.png){width=6.3in}
